...
namespace JonSkeet.DemoUtil
{
	...
	public class ApplicationChooser
	{
		...
		/// <param name="args">Arguments to pass in for methods which have a single string array parameter.</param>
		public static void Run(Type type, string[] args) => Run(type.GetTypeInfo().Assembly, args);
		...
		private static void Run(Assembly assembly, string[] args)
		{
			var options = assembly.DefinedTypes
				...
				.OrderBy(ep => GetDescription(ep!.DeclaringType)) // Not null here!
				.ThenBy(ep => ep!.DeclaringType.Name)             // Not null here!
			...
			while (true)
			{
				...
				{
					Console.WriteLine($"{option.Key}: {GetEntryPointName(methodBase: option.EntryPoint!)}"); // Not null here!
				}
				...
				while (entryPoint == null)
				{
					...
					entryPoint = options.FirstOrDefault(o => o.Key == key)?.EntryPoint;

					if (entryPoint == null) Console.Write("Invalid choice; please select again: ");
				}
				...
				try
				{
					...
					if (main.Invoke(null, main.GetParameters().Length == 0 ? null : new object[] { args }) is Task task) task.GetAwaiter().GetResult();
				}
				catch (Exception ex)
				{
					// Normally we fail due to an exception within the code invoked via reflection.
					// Unwrap the TargetInvocationException that would otherwise be wrapped in.
		...
		private static object GetEntryPointName(MethodBase methodBase)
		{
			...
			return description == null ? type.Name : $"{type.Name} [{description}]";
		}

		private static string? GetDescription(Type type) => type
			.GetTypeInfo()
		...
		internal static MethodBase? GetEntryPoint(TypeInfo type)
		{
			...
			var methods = type.DeclaredMethods.Where(m => m.IsStatic && m.Name == "Main" && !m.IsGenericMethodDefinition);
			...
			foreach (var method in methods)
			{
				...
				if (parameters.Length == 0) parameterless = method;
				else
					if (parameters.Length == 1 && parameters[0].ParameterType == typeof(string[])
											   && !parameters[0].IsOptional
											   && !parameters[0].IsOut) stringArrayParameter = method;
			}
